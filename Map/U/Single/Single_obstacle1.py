...
class MapGenerator:
    ...
    def generate_rectangle(self, img, density, start=None, end=None):
        """Generate an extended U-shaped obstacle at random position and direction, avoiding start/end points"""
        ...
        def is_safe_distance(x, y, width, height, start, end, safe_radius=20):
            ...
            # Obstacle boundary box
            obstacle_points = []

            # Add all boundary points based on direction
            for i in range(x, x + width + 1):
                obstacle_points.extend([(i, y), (i, y + height)])
            for i in range(y, y + height + 1):
                obstacle_points.extend([(x, i), (x + width, i)])

            # Check distance of each boundary point from start/end points
        ...
        def draw_u_shape(img, x, y, width, middle_length, thickness, direction):
            """Draw an extended U-shaped obstacle"""
            if direction == 0:  # Open at top
                ...
                cv2.rectangle(img, (x, y + middle_length - thickness),
                              (x + width, y + middle_length),
                              (0, 0, 0), -1)
                return (x, y, width, middle_length)  # Return bounding box
        ...
        u_shape_width = 80  # U-shape width
        middle_length = 160  # Vertical section length
        margin = 30  # Boundary margin
        safe_radius = 25  # Safe distance from start/end points

        max_attempts = 50  # Increase maximum attempts
        for attempt in range(max_attempts):
            img_temp = np.ones_like(img) * 255

            # Randomly select position and direction
            direction = random.randint(0, 3)

            # Determine available range based on direction
            ...
            # Check if safe distance is maintained from start/end points
            if direction in [0, 2]:
            ...
            # Check if traversable path exists
            if check_path_exists(img_temp, start, end):
                img[:] = img_temp
                return True

        # If all attempts fail, generate a simple U-shape away from start/end points
        img_temp = np.ones_like(img) * 255
        safe_x = margin
        safe_y = margin
        ...
        draw_u_shape(img_temp, safe_x, safe_y,
                     u_shape_width, middle_length,
                     wall_thickness, 0)  # Default open at top
        img[:] = img_temp
        return True

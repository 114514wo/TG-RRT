...
class MapGenerator:
    ...
    def generate_rectangle(self, img, density, start=None, end=None):
        ...
        # 定义基本的迷宫结构
        vertical_walls = [
            (40, 0, wall_thickness, 80),  # 左上
            (40, 120, wall_thickness, 104),  # 左下
            (120, 40, wall_thickness, 144),  # 中间
            (200, 80, wall_thickness, 144)  # 右侧
        ]

        horizontal_walls = [
            (0, 80, 100, wall_thickness),  # 左上横墙
            (60, 160, 80, wall_thickness),  # 左下横墙
            (140, 40, 84, wall_thickness),  # 右上横墙
            (120, 120, 100, wall_thickness)  # 右下横墙
        ]
        ...
        for attempt in range(max_attempts):
            ...
            # 随机选择要使用的墙壁
            selected_v_walls = random.sample(vertical_walls, k=random.randint(2, len(vertical_walls)))
            selected_h_walls = random.sample(horizontal_walls, k=random.randint(2, len(horizontal_walls)))
        ...
        img_temp = np.ones_like(img) * 255
        simple_walls = [
            (40, 0, wall_thickness, 80),
            (200, 80, wall_thickness, 144)
        ]
